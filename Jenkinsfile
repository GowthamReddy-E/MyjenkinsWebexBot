...
Files
/cim_trigger.jenkinsfile
//depot/firepower/ims/IMS_7_6_MAIN/feature-test-lib/tests/system/jenkinsfile/trigger_jobs/cim_trigger.jenkinsfile @=3641372
 View
 Commits
  Blame  Open  Edit  Download .zip  Download (10 KB)
                ...
                if(BAT.toBoolean()){
                    ...
                    for(job in jobs){
                        ...
                        parallelJobs[jobName] = {
                            ...
                            stage("Build ${jobName}") {
                                try {
                                    def buildResult = build(job: jobName, parameters: [[$class: 'StringParameterValue', name: 'BUILD', value: imsBuild]], propagate: false, wait: true)
                                    buildResults[jobName] = buildResult.result
                                    if (buildResult.result != 'SUCCESS') {
                                        error "Downstream job ${jobName} failed"
                                    }
                                } catch (Exception e) {
                                    buildResults[jobName] = 'FAILURE'
                                    error "Error while triggering or waiting for downstream job ${jobName}: ${e.message}"
                                }
                            }
                        }
                    }
